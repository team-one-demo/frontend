...
const BASE_URL = 'https://mcalc.ocpgitlab.com'
...
export const calculate = async (request: HomeLoanCalculateRequest): Promise<HomeLoanCalculateResponse> => {
  try {
    ...
    if (!response.ok) {
      throw createError(`Failed to calculate mortgage: ${response.statusText}`, response.status, response.statusText)
    }

    const data = await response.json()
    return data as HomeLoanCalculateResponse
  } catch (error) {
